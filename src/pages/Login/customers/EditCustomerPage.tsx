...
import { fetchCustomersForUser, updateCustomer } from '../../../services/customer/customerService';
...
const EditCustomerPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      ...
      if (!token || !id) return;
      const customers = await fetchCustomersForUser(token);
      const customer = customers.find((c: any) => c.customer_id === Number(id));
      if (customer) {
  ...
  const onSubmit = async (data: CustomerFormInputs) => {
    ...
    if (!token || !id) return;
    try {
      await updateCustomer(token, Number(id), data);
